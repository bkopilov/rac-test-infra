...
class BuildRunHammerDB(BuildRun):
    ...
    def env_params(cls):
        return """
        bash -c "cat > /opt/HammerDB-5.0/env_ << EOF
...
"
    ...
    def build_hammerbd(cls):
        return """
        bash -c "source /opt/HammerDB-5.0/env_ && /opt/HammerDB-5.0/hammerdbcli auto build.tcl"
        :return:
        """
    ...
    def run_hammerbd(cls):
        return """
        bash -c "source /opt/HammerDB-5.0/env_ && /opt/HammerDB-5.0/hammerdbcli auto run.tcl"
        :return:
        """
    ...
    def drop_hammerbd(cls):
        return """
        bash -c "source /opt/HammerDB-5.0/env_ && /opt/HammerDB-5.0/drop_tpcc_user.sh"
        """
